...
export default function ReporteInfraDetalle() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        setStatus({ type: 'info', text: 'Cargando reporte…' });
        ...
        if (!item) {
          setRow(null);
          setStatus({ type: 'error', text: 'No se encontró el reporte.' });
        } else {
          setRow(item);
          setStatus(null);
        }
        ...
        setRow(null);
        setStatus({ type: 'error', text: 'Error cargando el reporte.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={`${styles.page} ${styles.pageEnter}`}>
      ...
      <header className={styles.header}>
        <h1 className={styles.title}>Reporte de infraestructura #{id}</h1>
        {row?.estado ? (
        ...
        <>
          {isAdmin && !isResolved && (
            <div className={`${styles.card} ${styles.adminCallout}`}>
              <div className={styles.rowBetween}>
                ...
                <Link href={`/reporteinf/${id}/responder`} className={`${styles.btn} ${styles.btnWarn}`}>Ir a Responder</Link>
              </div>
            </div>
          )}
          ...
          <section className={`${styles.card} ${styles.animSection}`} aria-labelledby="sec-resumen">
            ...
            <div className={styles.metaGrid}>
              ...
              <div className={styles.metaItem}><span className={styles.metaLabel}>Tipo de reporte</span><span className={styles.metaValue}>{row.tipo_reporte || '—'}</span></div>
            </div>
          </section>
          ...
          <section className={`${styles.card} ${styles.animSection}`} aria-labelledby="sec-detalle">
            ...
            <div className={styles.detailList}>
              <div className={styles.detailRow}><span className={styles.detailKey}>Lugar</span><span className={styles.detailValue}>{row.lugar}</span></div>
              <div className={styles.detailRow} style={{ alignItems: 'start' }}><span className={styles.detailKey}>Reporte</span><span className={styles.detailValue} style={{ whiteSpace: 'pre-wrap' }}>{row.reporte}</span></div>
            </div>
          </section>
